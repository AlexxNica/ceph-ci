...
def pg_num_in_all_states(pgs, *states):
    return sum(1 for state in pgs.itervalues() if all(s in state for s in states))


def pg_num_in_any_state(pgs, *states):
    return sum(1 for state in pgs.itervalues() if any(s in state for s in states))
...
def test_create_from_mon(ctx, config):
    ...
    pg_states = manager.wait_till_pg_convergence()
...
def test_create_from_peer(ctx, config):
    ...
    from_primary = config.get('from_primary', 1)
    ...
    while True:
        pool_b = manager.create_pool_with_unique_name(pg_num)
        pg_states = manager.wait_till_pg_convergence(30)
        pg_created = pg_num_in_all_states(pg_states, 'active', 'clean')
        assert pg_created >= pg_num
        pg_pending = pg_num_in_any_state(pg_states, 'unknown', 'creating')
    ...
    pg_states = manager.wait_till_pg_convergence(30)
    pg_stuck = pg_num_in_any_state(pg_states, 'stale', 'peering')
    assert pg_stuck > 0
